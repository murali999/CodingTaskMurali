...
@interface CourcesViewController ()

@end

@implementation CourcesViewController
NSURLSessionConfiguration *config1;
NSURLSession *session1;
- (void)viewDidLoad {
    [super viewDidLoad];
    _responseData = [[NSArray alloc] init];
    config1 = [NSURLSessionConfiguration defaultSessionConfiguration];
    session1 = [NSURLSession sessionWithConfiguration:config1];
    [self getData];
}
...
-(void)getData{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSURLSessionDataTask *dataTask = [session1 dataTaskWithURL:[NSURL URLWithString:@"http://34.199.13.140/api/candidateMobileGetRecommendedCourseList/"] completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            NSError *parseError;
            _responseData = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:&parseError];
            dispatch_async(dispatch_get_main_queue(), ^{
                [_tableCourses reloadData];
            });
            
            NSLog(@"Courses data%@",_responseData);
        }];
        [dataTask resume];
    });
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGSize expectedLabelSize = [[data valueForKey:@"courseName"] sizeWithFont:lblArtName.font constrainedToSize:maximumLabelSize lineBreakMode:lblArtName.lineBreakMode];
    CGRect newFrame = lblArtName.frame;
    newFrame.size.height = expectedLabelSize.height;
    lblArtName.frame = newFrame;
    lblArtName.numberOfLines = 0;
    lblArtName.text = [[data valueForKey:@"courseName"] uppercaseString];
